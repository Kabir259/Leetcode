...
// Code-----------------------------------------------------------------------------------------------
int findmaxPath(ll node, vll& parents, vll& values, vll& vis, ll&ans){
    if(vis[node]) return values[node];
    if(parents[node]==-1) vis[node]=1;
    else{
        vis[node]=1;
        ll parentPath= findmaxPath(parents[node],parents,values,vis,ans);
        values[node] = max(values[node],values[node]+parentPath);
    }
    ans = max(ans,values[node]);
    return values[node];
}

int bestSumDownwardTreePath(vll parent, vll values){
    int n = parent.size();
    ll ans = values[0];
    vll vis(n, false);
    for (int i = 0; i < n; i++)
    {
        if (vis[i])
            continue;
        findmaxPath(i,parent,values,vis,ans);
    }
return ans;
}
