...
#include <bits/stdc++.h>
using namespace std;
ll maximumProfit(ll m, vector<ll> &earnings)
{
	ll n = earnings.size();
    vll reversed(n);

    fl(i,0,n){
        reversed[i] = earnings[(i+n/2)%n];
    }
    ll curr_earnings = 0;
    fl(i,0,m){
        curr_earnings+=earnings[i]+reversed[i];
    }
    ll profit = curr_earnings;
    fl(i,1,m){
        curr_earnings -= earnings[i-1]+reversed[i-1];
        curr_earnings+= earnings[(i+m-1)%n]+reversed[(i+m-1)%n];
        profit = max(profit,curr_earnings);
    }
    return profit;
}
ll main() {
	ll m = 2;
	vector<ll> earnings = {1, 5, 1, 3, 7, -3};
	cout << maximumProfit(m, earnings) << endl;
	return 0;
}
